...
int main(void){
	list2t p_node, p;
	p_node = (list2t) malloc(sizeof(Node2t));
	p_node->elem = 1;
	p = p_node;
	p->left = (list2t) malloc(sizeof(Node2t));
	p->right = NULL;
	p = p->left;
	p_node->elem = 2;
	p->left = NULL;
...
int count_level (list2t p_node, int level){
	...
	putst p_node;
	cur_level +=1;
	while(p_node->left != NULL && cur_level != level){
		if (p_node->left != NULL && cur_level != level){
			putst left
			cur_level += 1;
		} else {
			if (cur_level == level){
				...
				if (p_node->right == NULL && cur_level == 0){
					return amount_elem;
				} else {
